...
def make_dataset(path: str) -> Tuple[List[str], List[str]]:
    ...
    images_a        = []
    images_b        = []
    valid_img_ext   = [".bmp",".jpg",".jpeg","png"]
    for imgFile in os.listdir(path):
        ext = os.path.splitext(imgFile)[1]
        if ext.lower() not in valid_img_ext:
            continue
        if(imgFile[1:2].lower() == "a"):
            images_a.insert(int(imgFile[0:1]),os.path.join(path,imgFile))
        else:
            images_b.insert(int(imgFile[0:1]),os.path.join(path,imgFile))
    images_a.sort()
    images_b.sort()
...
def get_cutoff_frequencies(path: str) -> List[int]:
    ...
    ############################
    ### TODO: YOUR CODE HERE ###
    cutoff_frequencies = np.genfromtxt(path, delimiter="\n").astype(int)
...
class HybridImageDataset(data.Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, int]:
        ...
        ############################
        ### TODO: YOUR CODE HERE ###
        img = Image.open(self.images_a[idx])
        img2 = Image.open(self.images_b[idx])
        width, height = img.size
        img2 = img2.resize((width,height))
        image_a = self.transform(np.transpose(np.array(img),(0,1,2)))
        image_b = self.transform(np.transpose(np.array(img2),(0,1,2)))

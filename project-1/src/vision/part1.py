...
def create_Gaussian_kernel_2D(cutoff_frequency: int) -> np.ndarray:
    ...
    kernel2 = create_Gaussian_kernel_1D(cutoff_frequency * 4 + 1,cutoff_frequency)
...
def my_conv2d_numpy(image: np.ndarray, filter: np.ndarray) -> np.ndarray:
    ...
    redTemp         = np.zeros([rows, columns], dtype = int)
    blueTemp        = np.zeros([rows, columns], dtype = int)
    greenTemp       = np.zeros([rows, columns], dtype = int)
    ...
    red             = np.pad(red, pad_width = padding,mode='constant')
    blue            = np.pad(blue, pad_width = padding,mode='constant')
    green           = np.pad(green, pad_width = padding,mode='constant')

    for i in range(rows):
        for j in range(columns):
            redTemp[i][j]     = np.sum(red[i:i+ksizeRows,j:j+ksizeColumns]*filter)
            blueTemp[i][j]    = np.sum(blue[i:i+ksizeRows,j:j+ksizeColumns]*filter)
            greenTemp[i][j]   = np.sum(green[i:i+ksizeRows,j:j+ksizeColumns]*filter)
            
    filtered_image  = np.dstack((redTemp,greenTemp,blueTemp))

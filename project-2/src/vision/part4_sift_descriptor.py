...
def get_gradient_histogram_vec_from_patch(
    ...
    ###########################################################################
    wgh = []
    row = np.array(np.arange(0,16,1))
    magnitude = []
    orientation = []
    for i in range(0,4):
        columns = np.array(np.arange(4*i,4*(i+1),1))
        magnitude.append(np.array_split(window_magnitudes[row[:, np.newaxis],columns],4))
        orientation.append(np.array_split(window_orientations[row[:, np.newaxis],columns],4))
    magnitude = np.swapaxes(magnitude,0,1).reshape(16,4,4)
    orientation = np.swapaxes(orientation,0,1).reshape(16,4,4)
    for j in range(0,16):
        hist = np.histogram(orientation[j], bins = 8, range = (-np.pi, np.pi), weights = magnitude[j])
        wgh = np.append(wgh, hist[0])
    wgh = wgh.reshape(128, 1)
    wgh = torch.from_numpy(wgh)
    # 
...
def get_feat_vec(
    ...
    # TODO: YOUR CODE HERE                                                      #                                          #
    #############################################################################
    entry = (feature_width//2) - 1
    exitB = (feature_width//2) + 1
    result = []
    magnitude   = magnitudes[r - entry : r + exitB, c - entry: c + exitB]
    orientation = orientations[r - entry : r + exitB, c - entry : c  + exitB]
    fv = torch.Tensor.numpy(get_gradient_histogram_vec_from_patch(magnitude, orientation))
...
def get_SIFT_descriptors(
    ...
    for i in range(len(X)):
        res = get_feat_vec(X[i],Y[i],magnitudes,orientations).reshape(1,128)
        result.append(res)
    fvs = np.vstack(np.round(result,3))

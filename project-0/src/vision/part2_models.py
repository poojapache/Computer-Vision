...
class HybridImageModel(nn.Module):
    ...
    def get_kernel(self, cutoff_frequency: int) -> torch.Tensor:
        ...
        kernel = torch.Tensor(np.stack(self.n_channels*[[kernel]],axis=0))
    ...
    def low_pass(self, x: torch.Tensor, kernel: torch.Tensor):
        ...
        ksizeRows       = kernel.size()[2]
        ksizeColumns    = kernel.size()[3]
        filtered_image  = torch.nn.functional.conv2d(x,kernel,padding = (ksizeRows//2,ksizeColumns//2),groups = self.n_channels)
    ...
    def forward(
        ...
        filter            = self.get_kernel(torch.IntTensor.item(cutoff_frequency))
